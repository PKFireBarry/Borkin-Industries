...
export async function POST(request: NextRequest) {
  try {
    const { booking, failureReason }: { booking: Booking; failureReason?: string } = await request.json()
    ...
    // Send notification
    await sendPaymentFailureNotification(booking, client, contractor, services, failureReason)
